...
function casePage() {
  ...
  return `<section class="page case case-overview"><div class="case-top"><div><p class="kicker">${pack.identity.kicker}</p><h1>${escapeHelpHtml(display.title)}</h1><p>${icon("landmark")}${escapeHelpHtml(display.court)}</p><p>${term("cnr")}: <span class="record-value">${escapeHelpHtml(display.cnr)}</span></p><span class="case-status">${escapeHelpHtml(displayStatus)}</span><p class="record-note">${pack.identity.recordValues}</p><p class="sample-disclosure">${escapeHelpHtml(display.dataLabel || "Sample data - hackathon prototype. Not an official court record.")}</p></div><aside class="hearing-card"><span>${icon("calendar")} ${pack.agenda.next}</span><strong>${escapeHelpHtml(dateParts[0])}</strong><b>${escapeHelpHtml(dateParts[1])}</b><small>${escapeHelpHtml(displayNext)}</small><button type="button" class="btn" data-action="case-stage" data-stage="action">${journey[2]} &#8594;</button></aside></div><div class="case-grid"><div class="case-reading">${derivedMarkup}<section class="block record-block"><h2>${pack.record.heading}</h2><div class="order-modes" role="group" aria-label="${pack.record.heading}"><label><input type="radio" name="order-mode" checked> ${pack.record.meaning}</label><label><input type="radio" name="order-mode"> ${pack.record.official}</label></div><article class="record-meaning"><p>${escapeHelpHtml(explanations.meaning)}</p><button type="button" class="text-link" data-doc="0">${pack.documents.view} ${escapeHelpHtml(displayDocuments[0]?.title || pack.documents.items[0].title)} &#8599;</button></article></section><section class="block record-verify"><h2>${pack.record.verify}</h2><p>${escapeHelpHtml(explanations.verify)}</p></section><section class="block history-block"><h2>${pack.history.heading}</h2><div class="timeline">${displayHistory.map((item) => `<div><i class="dot"></i><span><b>${escapeHelpHtml(item.title)}</b><span>${escapeHelpHtml(item.detail)}</span></span></div>`).join("")}</div></section></div><aside class="case-rail"><section class="block documents-block"><h2>${pack.documents.heading}</h2>${displayDocuments.map((item, i) => `<div class="doc"><span><b>${escapeHelpHtml(item.title)}</b><span>${escapeHelpHtml(item.detail)}</span></span><button type="button" class="btn" data-doc="${i}">${pack.documents.view}</button></div>`).join("")}<button type="button" class="text-link case-papers-link" data-go="documents">${pack.documents.heading} &#8594;</button></section><button type="button" class="btn primary case-help" data-go="help" aria-label="${pack.support.accessible}">${pack.support.action}</button></aside></div></section>`;
}
...
function overlay() {
  ...
  if (state.menu) {
    o.innerHTML = `<div class="overlay menu-overlay" data-action="close-menu"><nav class="menu" role="dialog" aria-modal="true" aria-labelledby="menu-title" tabindex="-1"><header class="menu-header"><h2 id="menu-title">${tr("shared.mobileMenu.heading")}</h2><button type="button" class="menu-close" data-action="close-menu" aria-label="${tr("shared.actions.close")}">×</button></header>${mobileMenuItem("home", "home", tr("shared.nav.home"), "action")}${mobileMenuItem("finder", "search", tr("shared.nav.finder"))}${mobileMenuItem("courts", "landmark", tr("shared.nav.courts"))}${mobileMenuItem("documents", "folder", tr("shared.nav.documents"))}${mobileMenuItem("help", "circle-help", tr("shared.nav.help"))}${state.selected && state.profile ? mobileMenuItem("case", "briefcase", tr("shared.nav.workspace")) : ""}<button data-action="language">${icon("languages")}${languages[state.prefs.lang]}</button><button data-action="access">${icon("accessibility")}${tr("shared.accessibility.heading")}</button><button data-action="reset">${tr("shared.actions.reset")}</button></nav></div>`;
...
function applyCitizenHierarchy() {
  ...
  if (state.page === 'case') {
    ...
    history.closest('.citizen-disclosure')?.classList.add('history-disclosure');
...
function applyHash(hash) {
  ...
  const aliases = { paper: "finder", hearing: "case", understand: "help" };
...
function handleHelpSuggestion(control) {
  ...
  if (!detail) return;
  const knowledgeBase = detail.closest(".knowledge-base");
  if (knowledgeBase) knowledgeBase.open = true;
  detail.open = true;
